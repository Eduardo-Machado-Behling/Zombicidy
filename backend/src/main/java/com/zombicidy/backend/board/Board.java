...
import com.zombicidy.backend.board.scenery.*;
import java.util.*;
...
public class Board {
  ...
  private ArrayList<CommomZombie> toRemoveZombie =
      new ArrayList<CommomZombie>();
  ...
  public boolean IsValidMovement(int[] position) {
    ...
    if (deltaX <= movement && deltaX >= movement * -1 && deltaY <= movement &&
        deltaY >= movement * -1 && !(grid.GetType().equals("Wall"))) {
      return true;
    } else {
      return false;
    }
