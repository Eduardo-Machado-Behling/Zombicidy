...
public class ShaderManager {
  ...
  public void setUniform(String name, int val) {
    GL40.glUniform1i(getUniformLocation(name), val);
  }
  ...
  private String getSource(String name, ShaderType shaderType) {
    String filePath = "shaders/" + name + (shaderType == ShaderType.FRAGMENT ? ".fs" : ".vs");
    try (java.io.InputStream inputStream = ShaderManager.class.getClassLoader().getResourceAsStream(
        filePath)) {
      if (inputStream == null) {
