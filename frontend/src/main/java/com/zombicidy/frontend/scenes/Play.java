...
public class Play extends Scene {
  ...
  public Play(boolean debug) {
    ...
    generateUI();
    generateChestAssets();
    playerLight = new PointLight(new Vector3D(1.0f), new Vector3D(1.0f),
                                 new Vector3D(1.0f), new Vector3D(0, 1, 0),
                                 1.0f, 0.35f, 0.44f);
  ...
  public void init() {
    ...
    Engine.get().addLight(
        new DirectionalLight(new Vector3D(0, 2, 0), new Vector3D(0.8f),
  ...
  private void generateChestAssets() {
    ...
    String[] names =
        new String[] {"bandage_bau", "gun_bau", "bat_bau", "bullet_bau"};
  ...
  public void onKeyEvent(long window, int key, int scancode, int action,
    ...
    final Vector3D[] mirror =
        new Vector3D[] {new Vector3D(1, 1, 1).mult(cameraPos),
  ...
  public void onMouseEvent(long window, int button, int action, int mods) {
    ...
    if (paused) {
      if (button == 0 && action == GLFW.GLFW_PRESS) {
        ...
        Vector2D mousePos =
            new Vector2D((float)coords[0][0], (float)coords[1][0]);

        Vector2D btnRematch = pausePos.add(new Vector2D(0, 648 * pauseScale));
        Vector2D btnNew =
            pausePos.add(new Vector2D(445 * pauseScale, 648 * pauseScale));
        Vector2D btnRematchDiff = mousePos.sub(btnRematch);
        Vector2D btnNewDiff = mousePos.sub(btnNew);
        if (btnRematchDiff.x > 0 && btnRematchDiff.x < 370 &&
            btnRematchDiff.y > 0 && btnRematchDiff.y < 134) {
          paused = false;
          gos.clear();
          clean();
          UUID.clear();
          ...
          generateBoard(new Vector3D(0), scale);
          init();
        } else if (btnNewDiff.x > 0 && btnNewDiff.x < 370 && btnNewDiff.y > 0 &&
                   btnNewDiff.y < 134) {
  ...
  private void refreshAmmo() {
    ...
    Gun gun =
        Window.get().getEventListener().getBoard().getPlayer().getPlayerGun();
    int ammo = 0;
    if (gun != null) {
  ...
  public void GameWin() {
    ArrayList<Mesh2D.Vertex> vertices = new ArrayList<>();
    Vector2D pos =
        new Vector2D(Window.get().width() / 2, Window.get().height() / 2);
    TextureData data = AssetManager.get().getTexture("win");
    Vector2D size =
        new Vector2D(data.width * pauseScale, data.height * pauseScale);
