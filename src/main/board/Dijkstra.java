...
public class Dijkstra {
    ...
    public void FindPath( int[] position ) {
        ...
        temp[1] -= 1; 
        if( temp[1] >= 0 ) {
            if( FindLowestNeighbors( temp.clone() , pathMap[position[0]][position[1]].distance ) ) {
                FindPath( temp.clone() );
                return;
            }
        }
    ...
    public void CalculateNeighbors( int[] position , int[] nextPosition ) {
        if( pathMap[nextPosition[0]][nextPosition[1]] == null ) {
            if( board[nextPosition[0]][nextPosition[1]].GetType().equals("Ground") ) {
                pathMap[nextPosition[0]][nextPosition[1]] = new Node( pathMap[position[0]][position[1]].distance + 1 );
            ...
            if( nextPosition[0] == end[0] && nextPosition[1] == end[1] ) {
                if( pathMap[nextPosition[0]][nextPosition[1]].distance == -1 ) {
                    pathMap[nextPosition[0]][nextPosition[1]] = new Node( pathMap[position[0]][position[1]].distance + 1 );
                } else if( pathMap[nextPosition[0]][nextPosition[1]].distance > pathMap[position[0]][position[1]].distance + 1 ) {
                    pathMap[nextPosition[0]][nextPosition[1]] = new Node( pathMap[position[0]][position[1]].distance + 1 );
                }
                return;
            }
            if( pathMap[nextPosition[0]][nextPosition[1]].distance != 0 ) {
                if( pathMap[nextPosition[0]][nextPosition[1]].distance > pathMap[position[0]][position[1]].distance + 1 ) {
                    pathMap[nextPosition[0]][nextPosition[1]] = new Node( pathMap[position[0]][position[1]].distance + 1 );
                    CalculateDistance( nextPosition.clone() );
                }
            }
